...
export const Experience = () => {
  return (
    <section id="events" className="py-24 px-6 bg-[#0A0A0B] border-t border-white/10">
      <div className="max-w-7xl mx-auto">
        <motion.h2 
          initial={{ opacity: 0, x: -20 }}
          whileInView={{ opacity: 1, x: 0 }}
          viewport={{ once: true }}
          className="font-serif italic font-light tracking-tight text-4xl md:text-5xl mb-16 text-[#EBE6E0]"
        >
          The DIFF Experience
        </motion.h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-px bg-white/10 border border-white/10">
          {HIGHLIGHTS.map((item, index) => (
            <motion.div
              ...
              className="group relative h-48 bg-[#0A0A0B] p-6 overflow-hidden hover:bg-white/5 transition-colors cursor-pointer"
            >
              <div className="absolute top-6 left-6 text-[#EBE6E0] opacity-40 group-hover:opacity-100 group-hover:text-[#D4AF37] transition-all">
              ...
              <div className="absolute bottom-6 left-6 right-6 transform group-hover:-translate-y-2 transition-transform duration-300">
                <h3 className="font-sans font-bold uppercase tracking-wider text-xs text-[#EBE6E0] mb-2">{item.title}</h3>
                <p className="text-[10px] text-[#EBE6E0] opacity-0 group-hover:opacity-50 transition-opacity duration-300 line-clamp-2 font-mono">
                  {item.desc}
                </p>
              </div>
