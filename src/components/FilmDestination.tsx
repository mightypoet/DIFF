...
export const FilmDestination = () => {
  return (
    <section id="darjeeling" className="py-24 bg-[#0A0A0B] overflow-hidden border-t border-white/10">
      <div className="max-w-7xl mx-auto px-6 mb-16">
        <div className="mb-4 flex items-center gap-3">
          <div className="w-8 h-[1px] bg-[#D4AF37]"></div>
          <span className="uppercase tracking-[0.4em] text-[11px] text-[#D4AF37]">The Locations</span>
        </div>
        <h2 className="font-serif italic font-light tracking-tight text-4xl md:text-6xl lg:text-7xl leading-tight mb-6 text-[#EBE6E0]">
          Darjeeling <br /> 
          <span className="not-italic font-sans font-black uppercase tracking-tighter opacity-90 block mt-2">IS A FILM SET.</span>
        </h2>
        <p className="text-xl text-[#EBE6E0] opacity-60 font-serif italic pl-4 border-l border-white/20">
      ...
      <div className="flex overflow-x-auto no-scrollbar snap-x snap-mandatory px-6 pb-12 gap-8 w-full cursor-grab active:cursor-grabbing">
        {LOCATIONS.map((loc, idx) => (
          ...
          >
            <div className="aspect-[16/9] w-full overflow-hidden relative bg-[#1A1A1C] border border-white/10 p-2">
            ...
            <div className="flex flex-col mt-2">
              <span className="text-[10px] uppercase tracking-widest text-[#EBE6E0] opacity-40 font-mono mb-1">Location {loc.id}</span>
              <span className="font-sans font-bold uppercase tracking-widest text-sm text-[#EBE6E0]">{loc.title}</span>
            </div>
          </motion.div>
        ))}
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 mt-8 flex justify-end">
        <button className="flex items-center gap-3 text-[#EBE6E0] hover:text-[#D4AF37] transition-colors group text-[11px] font-bold tracking-[0.2em] uppercase">
          Discover Darjeeling for your next film
          <span className="w-8 h-[1px] bg-white/30 group-hover:bg-[#D4AF37] transition-colors"></span>
