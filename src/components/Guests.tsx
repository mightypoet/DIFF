        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {GUESTS.map((guest, idx) => (
            <motion.div
              ...
              className="group relative aspect-[3/4] overflow-hidden bg-[#F5F5F0] border border-black/10 cursor-pointer p-1"
            >
              <div className="w-full h-full relative overflow-hidden">
                ...
                />
                <div className="absolute inset-0 bg-gradient-to-t from-[#FFFFFF] via-[#FFFFFF]/40 to-transparent opacity-80 group-hover:opacity-100 transition-opacity duration-500" />
                ...
                <div className="absolute bottom-0 left-0 right-0 p-6 translate-y-8 group-hover:translate-y-0 transition-transform duration-500">
                  <h3 className="font-serif italic text-2xl text-[#0A0A0B] mb-1">{guest.name}</h3>
                  <p className="text-[#D4AF37] text-[10px] font-bold tracking-[0.2em] uppercase mb-2">{guest.role}</p>
                  <div className="h-[1px] w-0 bg-black/20 group-hover:w-full transition-all duration-700 delay-100 mb-2"></div>
                  <p className="text-[#0A0A0B] opacity-0 group-hover:opacity-60 font-sans text-xs transition-opacity duration-500 delay-200">
