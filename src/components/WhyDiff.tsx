    ...
    <section className="py-24 px-6 bg-[#FFFFFF] border-t border-black/10">
      <div className="max-w-7xl mx-auto">
        <motion.h2 
          ...
          viewport={{ once: true }}
          className="font-serif italic font-light tracking-tight text-4xl md:text-6xl mb-16 text-[#0A0A0B]"
        >
          Why DIFF?
        </motion.h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12">
          {REASONS.map((reason, index) => (
            <motion.div
              ...
              className="p-10 border border-black/10 bg-black/5 hover:bg-black/5 transition-colors group relative overflow-hidden"
            >
              ...
              <h3 className="font-sans font-black uppercase tracking-tighter text-xl md:text-2xl mb-4 text-[#0A0A0B] opacity-90">{reason.title}</h3>
              <p className="text-[#0A0A0B] opacity-60 font-serif italic text-lg leading-relaxed">{reason.description}</p>
            </motion.div>
          ))}
        </div>
      </div>
